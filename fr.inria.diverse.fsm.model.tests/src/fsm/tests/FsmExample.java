...
public class FsmExample {
	...
	public static void main(final String[] args) {
		...
		if (args.length == 0) {
			...
			try {
				final Resource resource = resourceSet.createResource(URI.createURI("http:///My.fsm"));
				final FSM root = FsmFactory.eINSTANCE.createFSM();
				...
				try {
					...
					for (final EObject eObject : resource.getContents()) {
						...
						if (diagnostic.getSeverity() != Diagnostic.OK) {
							FsmExample.printDiagnostic(diagnostic, "");
						}
					}
				}
	...
	protected static void printDiagnostic(final Diagnostic diagnostic, final String indent) {
		...
		for (final Diagnostic child : diagnostic.getChildren()) {
			FsmExample.printDiagnostic(child, indent + "  ");
		}
	}
