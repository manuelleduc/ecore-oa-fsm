...
public class TfsmExample {
	...
	public static void main(final String[] args) {
		...
		if (args.length == 0) {
			...
			try {
				final Resource resource = resourceSet.createResource(URI.createURI("http:///My.tfsm"));
				final TimedFSM root = TfsmFactory.eINSTANCE.createTimedFSM();
				...
				try {
					...
					for (final EObject eObject : resource.getContents()) {
						...
						if (diagnostic.getSeverity() != Diagnostic.OK) {
							TfsmExample.printDiagnostic(diagnostic, "");
						}
					}
				}
	...
	protected static void printDiagnostic(final Diagnostic diagnostic, final String indent) {
		...
		for (final Diagnostic child : diagnostic.getChildren()) {
			TfsmExample.printDiagnostic(child, indent + "  ");
		}
	}
